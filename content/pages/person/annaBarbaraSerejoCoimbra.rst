Anna Bárbara Serejo Coimbra
___________________________

:date: 2026-06-01 11:35
:modified: 2026-06-01 11:35
:slug: person/annaBarbaraSerejoCoimbra

.. AUTO-GENERATED GENERIC PROFILE: run build.py --update-publications

|

.. image:: {static}/images/person/generic.jpg
   :name: annaBarbaraSerejoCoimbra_face
   :width: 25%
   :alt: annaBarbaraSerejoCoimbra
   :align: left

Anna Bárbara Serejo Coimbra was an undergraduate scientific research student at `Polytechnic School`_/`UFRJ`_ under the supervision of `Prof. Gustavo R. Anjos`_. The project, *Pollutant Filtration in Multiphase Flows Resulting from Biofuel Combustion*, is associated with LabMFA research in computational mechanics, fluid dynamics, heat transfer, and numerical methods.

|
|
|
|
|

**academic info**:

 - Undergrad Scientific Research at `Polytechnic School`_/`Federal University of Rio de Janeiro`_
 - `Department of Mechanical Engineering`_
 - Project: *Pollutant Filtration in Multiphase Flows Resulting from Biofuel Combustion*
 - Year: 2023
 - Advisor: `Prof. Gustavo R. Anjos`_

|

**research interests**:

 - two-phase and multiphase flows
 - numerical methods
 - scientific computing

.. AUTO-GENERATED COAUTHORED ARTICLES START: run build.py --update-publications

.. role:: profile-author

.. raw:: html

   <style>
   .profile-author {
     background: rgba(0, 64, 133, 0.16) !important;
     border-radius: 4px;
     color: #004085 !important;
     font-weight: 700 !important;
     padding: 0 0.14rem;
   }
   </style>

**IC/TCC project**:

 - `Analysis of Filtration Efficiency in Biocombustion Emissions through Numerical Simulation on Unstructured Grids </documents/annaCoimbra-tcc.pdf>`_, Undergraduate research work (IC/TCC), Universidade Federal do Rio de Janeiro, 2023

**publications with Prof. Gustavo R. Anjos**:

Conference Papers
~~~~~~~~~~~~~~~~~

.. class:: publications-list conference-publications

#. :profile-author:`Coimbra, A. B. S.`; Anjos, G.R. **Analysis of Filtration Efficiency in Numerical Simulations of Biofuels Combustion**. *Anais do 20th Brazilian Congress of Thermal Sciences and Engineering* (ENCIT), Foz do Iguaçu, Brazil, 2024.

.. AUTO-GENERATED COAUTHORED ARTICLES END

.. Place your references here
.. _Prof. Gustavo R. Anjos: /person/gustavoRabello
.. _UFRJ: http://www.ufrj.br
.. _Federal University of Rio de Janeiro: http://www.ufrj.br
.. _Department of Mechanical Engineering: http://www.mecanica.ufrj.br/index.php/en/
.. _Polytechnic School: https://poli.ufrj.br/
