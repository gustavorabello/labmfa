João Deodato Batista dos Santos
_______________________________

:date: 2026-06-07 22:41
:modified: 2026-06-07 22:41
:slug: person/joaoDeodatoBatistaSantos

.. AUTO-GENERATED GENERIC PROFILE: run build.py --update-publications

|

.. image:: {static}/images/person/generic.jpg
   :name: joaoDeodatoBatistaSantos_face
   :width: 25%
   :alt: joaoDeodatoBatistaSantos
   :align: left

João Deodato Batista dos Santos was a post-doctoral researcher at `UFRJ`_/`Coppe`_ under the supervision of `Prof. Gustavo R. Anjos`_. The project, *Reduced-Order Modeling of Fluid-Structure Interaction*, is associated with LabMFA research in computational mechanics, fluid dynamics, heat transfer, and numerical methods.

|
|
|
|
|

**academic info**:

 - Post-Doc at `Coppe`_/`Federal University of Rio de Janeiro`_
 - `Department of Mechanical Engineering`_
 - Project: *Reduced-Order Modeling of Fluid-Structure Interaction*
 - Year: 2021
 - Advisor: `Prof. Gustavo R. Anjos`_

|

**research interests**:

 - fluid-structure interaction
 - numerical methods
 - scientific computing

.. AUTO-GENERATED COAUTHORED ARTICLES START: run build.py --update-publications

.. role:: profile-author

.. raw:: html

   <style>
   .profile-author {
     background: rgba(0, 64, 133, 0.16) !important;
     border-radius: 4px;
     color: #004085 !important;
     font-weight: 700 !important;
     padding: 0 0.14rem;
   }
   </style>

**publications with Prof. Gustavo R. Anjos**:

Journal Articles
~~~~~~~~~~~~~~~~

.. class:: publications-list journal-publications

#. :profile-author:`dos Santos, João D. B.`; Anjos, Gustavo R.; Savi, Marcelo A. `An investigation of fluid-structure interaction in pipe conveying flow using reduced-order models <https://doi.org/10.1007/s11012-022-01586-6>`_. *Meccanica*, vol. 57(10), 2473-2491 (2022).

.. AUTO-GENERATED COAUTHORED ARTICLES END

.. Place your references here
.. _Prof. Gustavo R. Anjos: /person/gustavoRabello
.. _UFRJ: http://www.ufrj.br
.. _Federal University of Rio de Janeiro: http://www.ufrj.br
.. _Department of Mechanical Engineering: http://www.mecanica.ufrj.br/index.php/en/
.. _Coppe: http://www.coppe.ufrj.br
