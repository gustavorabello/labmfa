Afrânio Augusto Gomes Gonçalves
_______________________________

:date: 2026-06-01 11:35
:modified: 2026-06-01 11:35
:slug: person/afranioAugustoGomesGoncalves

.. AUTO-GENERATED GENERIC PROFILE: run build.py --update-publications

|

.. image:: {static}/images/person/generic.jpg
   :name: afranioAugustoGomesGoncalves_face
   :width: 25%
   :alt: afranioAugustoGomesGoncalves
   :align: left

Afrânio Augusto Gomes Gonçalves is an M.Sc. student at `UFRJ`_/`Coppe`_ under the supervision of `Prof. Gustavo R. Anjos`_. The project, *Modelling of Fluid Flow Using Quadrilateral Unstructured Meshes*, is associated with LabMFA research in computational mechanics, fluid dynamics, heat transfer, and numerical methods.

|
|
|
|
|

**academic info**:

 - M.Sc. Student at `Coppe`_/`Federal University of Rio de Janeiro`_
 - `Department of Mechanical Engineering`_
 - Project: *Modelling of Fluid Flow Using Quadrilateral Unstructured Meshes*
 - Year: 2025
 - Advisor: `Prof. Gustavo R. Anjos`_

|

**research interests**:

 - computational fluid dynamics
 - numerical methods
 - scientific computing

.. Place your references here
.. _Prof. Gustavo R. Anjos: /person/gustavoRabello
.. _UFRJ: http://www.ufrj.br
.. _Federal University of Rio de Janeiro: http://www.ufrj.br
.. _Department of Mechanical Engineering: http://www.mecanica.ufrj.br/index.php/en/
.. _Coppe: http://www.coppe.ufrj.br
